...
export function BlockList({
  ...
  const mostUnusual = [...blocks]
    .sort((a, b) => comparison ? b.temperature - a.temperature : Math.abs(b.temperature - b.nearbyAverage) - Math.abs(a.temperature - a.nearbyAverage))
    .slice(0, comparison ? 5 : 8);
  ...
  return (
    <div
      ...
      role="group"
      aria-label={comparison ? "Select a cell with limited overnight cooling" : "Select a notable cell"}
    >
      <div className="mb-1 font-mono text-[9px] tracking-wide text-slate uppercase">{comparison ? "Least overnight cooling" : "Most unusual cells"}</div>
      <div className="flex flex-wrap gap-1.5">
      {choices.map((b) => {
        const anomaly = comparison ? b.temperature : b.temperature - b.nearbyAverage;
        ...
        return (
          ...
          >
              <span className="font-mono text-[10px] tracking-wide text-slate uppercase">{active ? "Selected" : comparison ? "Priority" : "Cell"}</span>
            <span
