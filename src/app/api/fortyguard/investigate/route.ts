...
export async function POST(request: Request) {
  ...
  try {
    const [heatmap, environment, satellite] = await Promise.all([
      submitJob("/heatmap", { polygon_aoi: localAoi(latitude!, longitude!), date_time: dateTime, granularity: 100, analytic_type: "tcm" }),
    ...
    return Response.json({
      ...
      jobs: [
        { kind: "heatmap", activityId: heatmap, label: "Local temperature comparison" },
