import { getMlEvidenceForDemoBlock, getMlEvidenceNearestTo, getMlModelInfo } from "@/lib/reasoning/mlExplain";
 ...
 *  - ?lat=&lng=  nearest-neighbor to one of the 17 collected LA AOIs, for the
...
export async function GET(request: Request) {
  ...
  if (blockId) {
    const evidence = getMlEvidenceForDemoBlock(blockId);
    if (!evidence) return Response.json({ error: "No ML evidence available for this block yet." }, { status: 404 });
    return Response.json({ evidence, model: getMlModelInfo() });
  }
  ...
  if (lat && lng) {
    ...
    return Response.json({ evidence: nearest.evidence, nearestAoi: nearest.aoi, model: getMlModelInfo() });
  }
