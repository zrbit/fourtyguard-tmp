...
export default function Home() {
  const [city, setCity] = useState<City>("Los Angeles"); const [blocks, setBlocks] = useState<BlockMetrics[]>([]); const [selectedId, setSelectedId] = useState(""); const [manualSelection, setManualSelection] = useState(false); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(true); const [loadStatus, setLoadStatus] = useState("Submitting live heatmap…"); const [investigate, setInvestigate] = useState(false);
  async function load() {
    setLoading(true); setLoadStatus("Loading cached live heatmap…"); setError(null); setBlocks([]); setInvestigate(false); setManualSelection(false);
  ...
  return <div className="flex h-dvh flex-col"><TopBar city={city} onCityChange={setCity} /><main className="flex min-h-0 flex-1">{loading ? <LiveMapLoading city={city} status={loadStatus} /> : error ? <div className="flex flex-1 flex-col items-center justify-center gap-4 p-6 text-center"><AlertTriangle className="h-7 w-7 text-thermal-hot" /><p className="max-w-md text-sm text-ash">{error}</p><button type="button" onClick={() => void load()} className="flex items-center gap-2 rounded border px-3 py-2 text-xs text-accent-strong" style={{ borderColor: "var(--accent-border)" }}><RefreshCw className="h-3.5 w-3.5" />Retry live scan</button></div> : block ? <><MapView blocks={blocks} selectedId={block.id} onSelect={(id) => { setSelectedId(id); setManualSelection(true); }} /><aside className="hidden w-[420px] flex-none overflow-y-auto border-l lg:block" style={{ borderColor: "var(--border)", background: "var(--ground)" }}><div className="p-5"><div className="flex items-center justify-between font-mono text-[11px] tracking-wider text-slate uppercase"><span>Selected 100 m cell</span><span className="flex items-center gap-1 text-accent-strong"><Radio className="h-3 w-3" />Live</span></div><div className="mt-5 flex items-end gap-2 font-mono"><span className="text-4xl font-semibold">{block.temperature.toFixed(1)}</span><span className="pb-1 text-lg text-ash">°F</span></div><p className="mt-2 font-mono text-[13px]" style={{ color: thermalColor(anomaly) }}>{formatSigned(anomaly, 2)}°F vs {block.nearbyBlockCount} nearest cells</p><p className="mt-2 font-mono text-[10.5px] text-slate">{block.lat.toFixed(5)}, {block.lng.toFixed(5)}</p><p className="mt-4 text-[12px] leading-relaxed text-slate">{manualSelection ? "You selected this cell. Click another heatmap marker to move the analysis." : "Automatically selected because this is the strongest local anomaly in the scan."}</p>{!investigate && <button type="button" onClick={() => setInvestigate(true)} className="mt-5 w-full rounded-lg py-2.5 text-sm font-bold" style={{ background: "var(--accent)", color: "var(--accent-ink)" }}>Add environmental evidence</button>}</div><LiveThermalReasoning block={block} allBlocks={blocks} />{investigate && <LiveInvestigation block={block} />}<InterventionSimulator block={block} /></aside></> : null}</main></div>;
}
